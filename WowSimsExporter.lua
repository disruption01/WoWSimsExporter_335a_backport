...
function WowSimsExporter:GetGearEnchantGems(type)
    ...
    for slotNum = 1, #slotNames do
        ...
        if itemLink then
        local _, Gem1Link = GetItemGem(GetInventoryItemLink("player", slotId), 1)
		if Gem1Link then Gem1 = select(3, strfind(Gem1Link, "item:(%d+)")) end
        local _, Gem2Link = GetItemGem(GetInventoryItemLink("player", slotId), 2)
		if Gem2Link then Gem2 = select(3, strfind(Gem2Link, "item:(%d+)")) end
        local _, Gem3Link = GetItemGem(GetInventoryItemLink("player", slotId), 3)
		if Gem3Link then Gem3 = select(3, strfind(Gem3Link, "item:(%d+)")) end
        local _, Gem4Link = GetItemGem(GetInventoryItemLink("player", slotId), 4)
		if Gem4Link then Gem4 = select(3, strfind(Gem4Link, "item:(%d+)")) end
        end
